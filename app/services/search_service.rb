...
#    statuses = StatusesIndex.filter(terms: { searchable_by: [account.id, 1, 26986] })
    statuses = StatusesIndex.query(multi_match: { type: 'most_fields', query: query, operator: 'and', fields: %w(text text.stemmed) })
                            .limit(limit).objects
